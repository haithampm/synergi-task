...
import { CalendarDays, FileText, Pencil, Plus, Search, TicketCheck } from 'lucide-react';
...
export default function TicketsPMORegister() {
  ...
  const buildPayload = () => {
    ...
    return {
      title,
      description: draft.descriptionCase,
      priority: draft.priority as WorkspaceTicket['priority'],
      status: draft.status as WorkspaceTicket['status'],
      assignee: draft.requestedBy || 'Unassigned',
      projectId: draft.projectId || undefined,
      sla: draft.status === 'closed' ? 'Closed' : 'Active',
      customFieldValues,
      idText: draft.idText,
      application: draft.application,
      requestedBy: draft.requestedBy,
  ...
  const updateStatus = async (ticket: WorkspaceTicket, status: string) => {
    ...
    await updateTicket.mutateAsync({
      id: ticket.id,
      status: status as WorkspaceTicket['status'],
      sla: status === 'closed' ? 'Closed' : 'Active',
      customFieldValues: {
        ...(ticket as any).customFieldValues,
        closureDate: closeDate,
      },
      closureDate: closeDate,
    } as any);
            ...
            <table className="min-w-[1900px] w-full text-sm">
              <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-3">Description / Case</th>
                  <th className="px-3 py-3">Priority</th>
                  <th className="px-3 py-3">Ticket Number</th>
                  <th className="px-3 py-3">Status</th>
                  <th className="px-3 py-3">Closure Date</th>
                  <th className="px-3 py-3">Reply</th>
                  <th className="px-3 py-3">Note 1</th>
                  <th className="px-3 py-3">Note 2</th>
                    ...
                    <tr key={ticket.id} className="border-t hover:bg-muted/20">
                      ...
                      <td className="px-3 py-3">
                        <Select value={ticket.status} onValueChange={(value) => updateStatus(ticket, value)}>
                          ...
                          <SelectContent>{ticketStatuses.map((status) => <SelectItem key={status} value={status}>{status.replace(/-/g, ' ')}</SelectItem>)}</SelectContent>
                        </Select>
                      </td>
                      <td className="px-3 py-3">{closureDate(ticket)}</td>
                      <td className="px-3 py-3 max-w-[260px]"><p className="line-clamp-2">{reply(ticket)}</p></td>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-5xl">
            ...
            <div className="grid gap-4 py-2 lg:grid-cols-3">
              ...
              <Select value={draft.projectId || '__none__'} onValueChange={(value) => setDraft((p) => ({ ...p, projectId: value === '__none__' ? '' : value }))}>
                <SelectTrigger><SelectValue placeholder="Project" /></SelectTrigger>
                <SelectContent><SelectItem value="__none__">No project</SelectItem>{projects.map((project: any) => <SelectItem key={project.id} value={project.id}>{project.name}</SelectItem>)}</SelectContent>
              </Select>
              <Input placeholder="Application" value={draft.application} onChange={(e) => setDraft((p) => ({ ...p, application: e.target.value }))} />
              <Select value={draft.requestedBy || '__manual__'} onValueChange={(value) => setDraft((p) => ({ ...p, requestedBy: value === '__manual__' ? '' : value }))}>
                <SelectTrigger><SelectValue placeholder="Requested By" /></SelectTrigger>
                <SelectContent><SelectItem value="__manual__">Manual entry</SelectItem>{teamMembers.map((member: any) => <SelectItem key={member.id} value={member.name}>{member.name}</SelectItem>)}</SelectContent>
              </Select>
              ...
              <div className="lg:col-span-3">
                <Textarea rows={4} placeholder="Description (Case) *" value={draft.descriptionCase} onChange={(e) => setDraft((p) => ({ ...p, descriptionCase: e.target.value }))} />
              </div>
              <Select value={draft.priority} onValueChange={(value) => setDraft((p) => ({ ...p, priority: value }))}>
              ...
              <div className="lg:col-span-2">
                <Textarea rows={3} placeholder="Reply" value={draft.reply} onChange={(e) => setDraft((p) => ({ ...p, reply: e.target.value }))} />
              </div>
              <Textarea rows={3} placeholder="Note 1" value={draft.note1} onChange={(e) => setDraft((p) => ({ ...p, note1: e.target.value }))} />
              <Textarea rows={3} placeholder="Note 2" value={draft.note2} onChange={(e) => setDraft((p) => ({ ...p, note2: e.target.value }))} />
              <div className="rounded-2xl border bg-muted/20 p-4 text-sm lg:col-span-3">
                <div className="flex items-center gap-2 font-semibold"><TicketCheck className="h-4 w-4 text-primary" /> Ticket Register Mapping</div>
                <p className="mt-1 text-muted-foreground">The register fields are saved with the ticket record and mirrored into custom field values so import/export and future form changes can reuse them.</p>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
              <Button onClick={saveTicket}>{editingTicket ? 'Save Ticket Case' : 'Create Ticket Case'}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>

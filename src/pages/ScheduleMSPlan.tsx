...
type SortKey = 'wbs' | 'name' | 'startDate' | 'endDate' | 'progress' | 'critical';
...
const Timeline = ({ row, start, span }: { row: Row; start: string; span: number }) => {
  ...
  const width = row.durationDays === 0 ? Math.max(4, (daysBetween(row.startDate, row.endDate) / Math.max(1, span)) * 100) : (daysBetween(row.startDate, row.endDate) / Math.max(1, span)) * 100;
...
const ScheduleMSPlan = () => {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    projectsToShow.forEach((project, projectIndex) => {
      const projectRows = searchFilteredTasks.filter((row) => row.projectId === project.id);
      ...
      const projectSummary: SummaryRow = {
        ...
        endDate,
        durationDays: 0,
        progress: projectRows.length ? Math.round((completed / projectRows.length) * 100) : Number(project.progress ?? 0),
      ...
      phases.forEach((phase, phaseIndex) => {
        ...
        const phaseStarts = phaseRows.map((row) => row.startDate).sort();
        const phaseEnds = phaseRows.map((row) => row.endDate).sort();
        const phaseCompleted = phaseRows.filter((row) => row.status === 'done').length;
        ...
        const groupRow: SummaryRow = {
          ...
          title: phase,
          phase,
          startDate: phaseStarts[0],
          endDate: phaseEnds.at(-1) ?? phaseStarts[0],
          durationDays: 0,
          progress: Math.round((phaseCompleted / phaseRows.length) * 100),
          plannedHours: phaseRows.reduce((sum, row) => sum + Number(row.plannedHours || 0), 0),
          totalActivities: phaseRows.length,
          completedActivities: phaseCompleted,
    ...
    return output;
  }, [collapsed, projectFilter, projects, query, searchFilteredTasks, sortDirection, sortKey]);

  const timelineStart = rows.map((row) => row.startDate).sort()[0] ?? today;
  const timelineEnd = rows.map((row) => row.endDate).sort().at(-1) ?? addDays(today, 30);
  ...
  const exportCsv = () => {
    const lines = [['WBS', 'Task Name', 'Duration', 'Start', 'Finish', 'Predecessors', 'Resource Names', '% Complete', 'Critical'], ...rows.map((row) => [row.wbs, row.title, row.durationDays === 0 ? '0d' : `${row.durationDays}d`, row.startDate, row.endDate, isTask(row) ? (taskRows.find((item) => item.id === row.predecessorId)?.wbs ?? '') : '', '', `${row.progress}%`, isTask(row) && row.critical ? 'Yes' : row.kind !== 'task' && row.criticalCount ? 'Yes' : 'No'])];
  ...
  const renderRow = (row: Row) => {
    ...
    return <tr key={row.id} style={{ height: rowHeight }} className={`${row.kind === 'project' ? 'bg-slate-900 text-white' : row.kind === 'group' ? 'bg-blue-50 font-bold dark:bg-blue-950/30' : 'odd:bg-muted/20'} ${isDirty ? 'outline outline-1 outline-amber-400' : ''} ${isTask(row) && row.critical ? 'ring-1 ring-red-500/30' : ''}`}>
      <td className="border-b px-2 py-0 text-[11px] font-mono"><span style={{ paddingLeft: `${row.level * 14}px` }} className="inline-flex items-center gap-1">{summary ? <button className={`rounded border px-1 font-black ${row.kind === 'project' ? 'border-white/30 text-cyan-300' : 'text-primary'}`} onClick={() => setCollapsed((current) => current.includes(row.id) ? current.filter((id) => id !== row.id) : [...current, row.id])}>{collapsedState ? '+' : '-'}</button> : <span className="w-5" />}{row.wbs}</span></td>
      <td className="min-w-[320px] border-b px-2 py-0 text-xs">{row.kind === 'project' ? <Link to={`/projects?projectId=${row.projectId}`} className="font-black text-cyan-300 underline-offset-4 hover:underline">{row.title} · {row.totalActivities} activities</Link> : row.kind === 'group' ? <span className="font-black text-blue-800 dark:text-blue-200">{row.title} · {row.totalActivities} activities</span> : mode === 'edit' ? <Input value={row.title} onChange={(event) => updateDraft(row.id, { title: event.target.value })} className={cellInput} /> : <Link to={`/tasks?taskId=${row.id}&projectId=${row.projectId}`} className={`underline-offset-4 hover:underline ${row.critical ? 'text-red-600' : 'text-foreground'}`}>{row.critical ? <TriangleAlert className="mr-1 inline h-3 w-3" /> : null}{row.title}</Link>}</td>
      <td className="border-b px-2 py-0 text-xs">{row.durationDays === 0 ? '0d' : `${row.durationDays}d`}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) && mode === 'edit' ? <Input type="date" value={row.startDate} onChange={(event) => updateDraft(row.id, { startDate: event.target.value })} className={cellInput} /> : fmt(row.startDate)}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) && mode === 'edit' ? <Input type="date" value={row.endDate} onChange={(event) => updateDraft(row.id, { endDate: event.target.value })} className={cellInput} /> : fmt(row.endDate)}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) ? (mode === 'edit' ? <select value={row.predecessorId ?? ''} onChange={(event) => updateDraft(row.id, { predecessorId: event.target.value || undefined })} className={`${cellInput} w-40`}><option value="">None</option>{taskRows.filter((item) => item.id !== row.id).map((item) => <option key={item.id} value={item.id}>{item.wbs} {item.title}</option>)}</select> : (taskRows.find((item) => item.id === row.predecessorId)?.wbs ?? '-')) : '-'}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) && mode === 'edit' ? <Input type="number" min="0" value={row.plannedHours} onChange={(event) => updateDraft(row.id, { plannedHours: Number(event.target.value) })} className={`${cellInput} w-16`} /> : `${row.plannedHours}h`}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) && mode === 'edit' ? <select value={row.status} onChange={(event) => updateDraft(row.id, { status: event.target.value as Status })} className={cellInput}><option value="backlog">Backlog</option><option value="todo">Tasks</option><option value="in-progress">In Progress</option><option value="review">Review</option><option value="done">Done</option></select> : isTask(row) ? statusLabel(row.status) : `${row.completedActivities}/${row.totalActivities}`}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) && mode === 'edit' ? <Input type="number" min="0" max="100" value={row.progress} onChange={(event) => updateDraft(row.id, { progress: Number(event.target.value) })} className={`${cellInput} w-16`} /> : `${row.progress}%`}</td>
      <td className="border-b px-2 py-0"><Timeline row={row} start={timelineStart} span={timelineSpan} /></td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) ? (row.critical ? <span className="rounded bg-red-500/10 px-1 font-bold text-red-600">Critical</span> : '-') : row.criticalCount ? <span className="rounded bg-red-500/10 px-1 font-bold text-red-600">{row.criticalCount}</span> : '-'}</td>
      <td className="border-b px-2 py-0 text-xs">{isTask(row) ? <select value="" onChange={(event) => { doAction(row, event.target.value); event.target.value = ''; }} className="h-6 rounded border bg-background px-1 text-xs"><option value="">Action</option><option value="sub">Add task in phase</option><option value="earlier">Move earlier</option><option value="later">Move later</option><option value="milestone">Toggle milestone</option></select> : <Button size="sm" variant={row.kind === 'project' ? 'secondary' : 'outline'} onClick={() => addTask(row.kind === 'group' ? row.phase : 'Execution')}>Add</Button>}</td>
    </tr>;
  };

  return <AppLayout><AppHeader title="Master Schedule" subtitle="MS Project-style plan: no Project column, collapsible project/phase summary rows, 0d group rows, roll-up totals, timeline bars, predecessors, and Save All." />
    <div className="space-y-4 p-4 sm:p-6">
      <div className="rounded-[1.5rem] bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 p-4 text-white shadow-xl"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-300">MS Project Plan Schedule</p><h1 className="text-xl font-black">Plan View Without Project Column</h1><p className="text-xs text-slate-300">Project names appear as collapsible summary rows. Phase groups are 0d summary rows. Child tasks roll up to totals exactly like a project plan.</p></div><div className="flex flex-wrap gap-2"><Button variant="secondary" size="sm" onClick={() => addTask()}><Plus className="mr-1 h-4 w-4" /> Task</Button><Button variant="secondary" size="sm" onClick={() => addTask('Execution', true)}><Milestone className="mr-1 h-4 w-4" /> Milestone</Button><Button variant="secondary" size="sm" onClick={() => void generatePlan()}><Sparkles className="mr-1 h-4 w-4" /> Generate</Button><Button variant="secondary" size="sm" onClick={exportCsv}><Download className="mr-1 h-4 w-4" /> Export</Button><Button size="sm" onClick={() => void saveAll()} disabled={!dirtyCount}><Save className="mr-1 h-4 w-4" /> Save All ({dirtyCount})</Button></div></div></div>
      ...
      <div className="grid gap-3 sm:grid-cols-5">{[['Rows', rows.length], ['Activities', searchFilteredTasks.length], ['Critical', searchFilteredTasks.filter((row) => row.critical).length], ['Milestones', searchFilteredTasks.filter((row) => row.isMilestone).length], ['Dirty', dirtyCount]].map(([label, value]) => <div key={label} className="rounded-xl border bg-background p-3"><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{label}</p><p className="text-xl font-black">{value}</p></div>)}</div>
      {view === 'gantt' ? <div className="rounded-2xl border bg-card p-3"><div className="space-y-1 overflow-auto" style={{ maxHeight: viewportHeight }}>{rows.map((row) => <div key={row.id} className={`grid min-w-[940px] grid-cols-[360px_1fr] items-center gap-2 px-2 text-xs ${row.kind === 'project' ? 'bg-slate-900 py-1 text-white' : row.kind === 'group' ? 'bg-blue-50 py-1 font-bold dark:bg-blue-950/30' : ''}`}><span>{row.kind !== 'task' ? <button className="mr-2 rounded border px-1" onClick={() => setCollapsed((current) => current.includes(row.id) ? current.filter((id) => id !== row.id) : [...current, row.id])}>{collapsed.includes(row.id) ? '+' : '-'}</button> : null}{row.kind === 'project' ? <Link to={`/projects?projectId=${row.projectId}`} className="font-bold text-cyan-300">{row.title}</Link> : isTask(row) ? <Link to={`/tasks?taskId=${row.id}&projectId=${row.projectId}`} className={row.critical ? 'font-bold text-red-600' : 'font-bold text-primary'}>{row.wbs} · {row.title}</Link> : `${row.wbs} · ${row.title}`}</span><Timeline row={row} start={timelineStart} span={timelineSpan} /></div>)}</div></div> : <div className="rounded-2xl border bg-card shadow-sm"><div className="overflow-auto" style={{ maxHeight: viewportHeight }}><table className="min-w-[1700px] border-separate border-spacing-0 text-left text-xs"><thead><tr>{header('WBS', 'wbs')}{header('Task Name', 'name')}<th className="sticky top-0 z-20 border-b bg-background px-2 py-1 text-left text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Duration</th>{header('Start', 'startDate')}{header('Finish', 'endDate')}<th className="sticky top-0 z-20 border-b bg-background px-2 py-1 text-left text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Predecessors</th><th className="sticky top-0 z-20 border-b bg-background px-2 py-1 text-left text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Work</th><th className="sticky top-0 z-20 border-b bg-background px-2 py-1 text-left text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Status</th>{header('% Complete', 'progress')}<th className="sticky top-0 z-20 border-b bg-background px-2 py-1 text-left text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Timeline</th>{header('Critical', 'critical')}<th className="sticky top-0 z-20 border-b bg-background px-2 py-1 text-left text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground">Action</th></tr></thead><tbody>{visibleRows.map(renderRow)}</tbody></table></div></div>}
    </div></AppLayout>;
};

export default ScheduleMSPlan;

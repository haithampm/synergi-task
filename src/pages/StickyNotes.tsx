...
const StickyNotesPage = () => {
  ...
  return (
    <AppLayout>
      <AppHeader title="Sticky Notes" subtitle="Simple quick notes, reminders, and pinned actions." />
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {sortedStickyNotes.length === 0 ? (
            ...
            sortedStickyNotes.map((note) => (
              <Card key={note.id} className={`glass border-border/70 shadow-sm transition-shadow hover:shadow-md ${note.done ? "opacity-70" : ""}`}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {note.done ? <Badge variant="secondary">Done</Badge> : <Badge variant="outline">Open</Badge>}
                      </div>
                      <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{note.content}</p>
                    </div>
                    <span className="shrink-0 rounded-full bg-muted px-2 py-1 text-[10px] font-semibold text-muted-foreground">{note.createdAt ? new Date(note.createdAt).toLocaleDateString() : "Today"}</span>
                  </div>

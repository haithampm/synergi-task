...
type DatasetKey = "projects" | "tasks" | "tickets" | "teamMembers" | "userAccounts" | "meetings" | "stickyNotes";
type ImportMode = "merge" | "replace";
type StepKey = "idle" | "reading" | "parsing" | "validating" | "saving" | "refreshing" | "complete" | "error";

const datasets: Array<{ key: DatasetKey; label: string; required: string[]; prefix: string }> = [
  { key: "projects", label: "Projects", required: ["name"], prefix: "project" },
  { key: "tasks", label: "Tasks", required: ["title"], prefix: "task" },
  { key: "tickets", label: "Tickets", required: ["title"], prefix: "ticket" },
  { key: "teamMembers", label: "Team Members", required: ["name", "email"], prefix: "member" },
...
const cleanRecord = (record: Record<string, unknown>) =>
  Object.fromEntries(Object.entries(record).map(([key, value]) => [key, parseValue(value)]).filter(([, value]) => value !== undefined && value !== ""));

const ensureId = (value: unknown, prefix: string) => typeof value === "string" && value.trim() ? value.trim() : makeId(prefix);

const normalizeRecord = (dataset: DatasetKey, record: Record<string, unknown>) => {
  ...
  switch (dataset) {
    ...
    case "meetings":
      return { id, title: data.title, type: data.type ?? "Planning", attendeeIds: [], startsAt: data.startsAt ?? now, endsAt: data.endsAt ?? data.startsAt ?? now, provider: data.provider ?? "workspace", status: data.status ?? "scheduled", notes: data.notes ?? "" };
    case "stickyNotes":
...
const stepLabels: Record<StepKey, string> = {
  ...
  validating: "Validating required fields",
  saving: "Saving workspace data",
  refreshing: "Refreshing dashboard counts",
...
const ImportExportProgress = () => {
  ...
  const selectedDataset = datasets.find((item) => item.key === dataset) ?? datasets[0];
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      if (ext === "csv") rawRecords = parseCsv(text);
      else if (ext === "json") {
        const parsed = JSON.parse(text);
        rawRecords = Array.isArray(parsed) ? parsed : Array.isArray(parsed[dataset]) ? parsed[dataset] : [];
      } else {
        throw new Error("Unsupported file. Please import CSV or JSON.");
      }

      if (!rawRecords.length) throw new Error("No rows found in the selected file.");
      ...
      updateWorkspaceData((current: WorkspaceData) => ({
        ...current,
        [dataset]: mode === "replace" ? normalized : upsertRecords((current[dataset] as any[]) ?? [], normalized),
        auditLogs: [{ id: makeId("audit"), action: "Workspace file imported", entityType: "settings", entityId: dataset, actorName: current.settings.currentUser.displayName || current.settings.profile.email || "Workspace User", detail: `${normalized.length} ${selectedDataset.label.toLowerCase()} imported from ${file.name}.`, createdAt: new Date().toISOString() }, ...current.auditLogs].slice(0, 300),
      }));

      setImportStep("refreshing", 92);
      await refreshWorkspace();
      setResult({ accepted: normalized.length, skipped: errors.length, errors: errors.slice(0, 10) });
      setImportStep("complete", 100);
      toast.success(`Import completed: ${normalized.length} saved${errors.length ? `, ${errors.length} skipped` : ""}.`);
    } catch (error) {
      setStep("error");
      setProgress(100);
  ...
  const exportDataset = (format: "csv" | "json") => {
    ...
    const records = (current[dataset] as unknown as Record<string, unknown>[]) ?? [];
    if (!records.length) {
      toast.error(`No ${selectedDataset.label.toLowerCase()} to export.`);
      return;
    }
  ...
  return (
    <AppLayout>
      <AppHeader title="Import / Export" subtitle="Import files with visible loading progress, validation, and safe workspace save." />
      <div className="space-y-6 p-4 sm:p-6">
        <PageSection title="Import File Progress Center" description="Choose a dataset, upload CSV or JSON, then watch each import step from reading to save completion." />

        <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          <Card className="glass border-primary/20 shadow-xl">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg"><UploadCloud className="h-5 w-5 text-primary" /> Import data file</CardTitle>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label>Dataset</Label>
                  <Select value={dataset} onValueChange={(value) => setDataset(value as DatasetKey)} disabled={importing}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{datasets.map((item) => <SelectItem key={item.key} value={item.key}>{item.label}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  <Label>Import mode</Label>
                  <Select value={mode} onValueChange={(value) => setMode(value as ImportMode)} disabled={importing}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent><SelectItem value="merge">Merge / update existing</SelectItem><SelectItem value="replace">Replace selected dataset</SelectItem></SelectContent>
                  </Select>
                </div>
              </div>

              <div className="rounded-3xl border border-dashed border-primary/40 bg-primary/5 p-5">
                <Input ref={inputRef} type="file" accept=".csv,.json,text/csv,application/json" disabled={importing} onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleImport(file); }} />
                <p className="mt-3 text-sm text-muted-foreground">Required fields: <span className="font-semibold text-foreground">{selectedDataset.required.join(", ")}</span></p>
              </div>

              <div className={`rounded-3xl border p-5 transition-all ${importing || result ? "border-primary/30 bg-primary/5" : "border-border bg-muted/10"}`}>
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Badge variant={step === "error" ? "destructive" : step === "complete" ? "default" : "secondary"}>{Math.round(progress)}%</Badge>
                </div>
                <Progress value={progress} className="h-3" />
                <div className="mt-3 grid grid-cols-4 gap-2 text-[11px] text-muted-foreground">
                  <span className={progress >= 8 ? "font-bold text-primary" : ""}>Read</span>
                  <span className={progress >= 30 ? "font-bold text-primary" : ""}>Parse</span>
                  <span className={progress >= 55 ? "font-bold text-primary" : ""}>Validate</span>
                  <span className={progress >= 78 ? "font-bold text-primary" : ""}>Save</span>
                </div>
              </div>

              {result && (
                <div className="rounded-2xl border bg-muted/20 p-4">
                  <div className="flex flex-wrap gap-2"><Badge className="gap-1"><CheckCircle2 className="h-3 w-3" /> {result.accepted} accepted</Badge><Badge variant={result.errors.length ? "destructive" : "secondary"} className="gap-1"><AlertCircle className="h-3 w-3" /> {result.errors.length} issues</Badge></div>
                  {result.errors.length > 0 && <ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-muted-foreground">{result.errors.map((error) => <li key={error}>{error}</li>)}</ul>}
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="glass">
            <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Download className="h-5 w-5 text-primary" /> Export backup</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">Export the selected dataset for review, cleanup, or backup before import.</p>
              <div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => exportDataset("csv")}><Download className="mr-2 h-4 w-4" /> CSV</Button><Button variant="outline" onClick={() => exportDataset("json")}><Download className="mr-2 h-4 w-4" /> JSON</Button></div>
              <Button variant="ghost" className="w-full" onClick={() => void refreshWorkspace()}><RefreshCw className="mr-2 h-4 w-4" /> Refresh counts</Button>
            </CardContent>
          </Card>
        </div>

        <Card className="glass overflow-hidden">
          <CardHeader><CardTitle className="flex items-center gap-2"><FileSpreadsheet className="h-5 w-5 text-primary" /> Workspace counts</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader><TableRow><TableHead>Dataset</TableHead><TableHead>Required fields</TableHead><TableHead className="text-right">Count</TableHead></TableRow></TableHeader>
              <TableBody>{datasets.map((item) => <TableRow key={item.key}><TableCell className="font-semibold">{item.label}</TableCell><TableCell className="text-muted-foreground">{item.required.join(", ")}</TableCell><TableCell className="text-right font-black">{counts[item.key]}</TableCell></TableRow>)}</TableBody>
            </Table>
          </CardContent>
        </Card>
      </div>

...
export default function ProjectTaskBulkActionsPanel() {
  ...
  const deleteSelected = async () => {
    ...
    if (!ok) return;
    for (const task of selectedTasks) await deleteTask.mutateAsync(task.id);
  ...
  const triggerButton = (
    <Button className="gap-2 shadow-sm" variant={pathname === "/tasks" || pathname === "/schedule" ? "default" : "default"}>
    ...
    <div className={pathname === "/tasks" || pathname === "/schedule" ? "sticky top-3 z-30 mx-4 mb-3 rounded-2xl border bg-card/95 p-3 shadow-lg backdrop-blur print:hidden sm:mx-6" : "fixed bottom-5 right-5 z-40 print:hidden"}>
      <Dialog open={open} onOpenChange={setOpen}>
        <div className={pathname === "/tasks" || pathname === "/schedule" ? "flex flex-wrap items-center justify-between gap-3" : ""}>
          {(pathname === "/tasks" || pathname === "/schedule") ? <div><p className="text-xs font-black uppercase tracking-widest text-primary">Edit / Delete Tasks</p><p className="text-sm text-muted-foreground">Select tasks, bulk edit status/priority/owner, or delete selected tasks.</p></div> : null}
          <DialogTrigger asChild>{triggerButton}</DialogTrigger>
        </div>
        <DialogContent className="max-w-6xl">
          <DialogHeader><DialogTitle>{pageLabel} Task Edit / Delete Actions</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div className="grid gap-3 lg:grid-cols-[260px_1fr]">
              ...
              <div className="flex flex-wrap items-center gap-2 rounded-xl border bg-muted/20 p-2 text-sm"><Badge variant="secondary">{projectTasks.length} visible tasks</Badge><Badge>{selectedTasks.length} selected</Badge><Button variant="outline" size="sm" onClick={selectAllVisible}>Select All</Button><Button variant="ghost" size="sm" onClick={clearSelection}>Clear</Button></div>
            </div>
